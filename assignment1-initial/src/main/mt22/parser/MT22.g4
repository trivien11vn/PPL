...
funcpro: ID COLON FUNCTION (ato_typ | void_typ |auto_typ | arr_typ) LB paralist RB (INHERIT ID)?;
paralist: paraprime | ;
...
block_stmt: LP normal_stmtlist RP;
normal_stmtlist: (normal_stmt|block_stmt) normal_stmtlist | ;
normal_stmt: vardecl
	|assignstmt
	|ifstmt
	|forstmt
	...
	|callstmt
	;
assignstmt: (ID | ID dimension) ASSIGN expr SEMI;
ifstmt: IF LB expr RB stmt (ELSE stmt)?;
forstmt: FOR LB (ID | ID dimension) ASSIGN expr COMMA expr COMMA expr RB stmt;
...
expr7: ID dimension |expr8;
dimension: LSB exprprime RSB;
...
//types
arr_typ: 'array' LSB INT_LITERAL (',' INT_LITERAL)* RSB OF ato_typ;
...
array_literal: LP index? (COMMA index)* RP;
index: expr;
